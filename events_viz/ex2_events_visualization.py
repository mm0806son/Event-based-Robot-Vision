...
# %% Sensor size
# img_size = (height, width), e.g., for DAVIS sensor size
img_size = (180, 240)
...
img_pos = np.zeros(img_size, int)
img_neg = np.zeros(img_size, int)
for i in range(num_events):
    if pol[i] > 0:
        img_pos[y[i], x[i]] += 1  # count positive events
    else:
        img_neg[y[i], x[i]] += 1  # count negative events
...
hist3d = np.zeros(img.shape + (num_bins,), int)
for ii in range(m):
